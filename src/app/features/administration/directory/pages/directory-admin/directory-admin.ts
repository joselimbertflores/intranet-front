...
export default class DirectoryAdmin implements OnInit {
  ...
  openDirectoryDialog(directory?: DirectoryEntryResponse): void {
    ...
    //   header: directory
    //     ? 'Editar destino telefónico'
    //     : 'Agregar destino telefónico',
    //   modal: true,
    //   draggable: false,
    //   closeOnEscape: true,
    //   closable: true,
  ...
  removeEntry(entry: DirectoryEntryResponse): void {
    ...
    //   accept: () =>
    //     this.dataSource.remove(entry.id).subscribe(() => this.loadEntries()),
    // });
  }
  ...
  removeSite(site: DirectorySite): void {
    // this.confirmationService.confirm({
    //   message:
    //     'Los destinos asociados conservarán sus teléfonos, pero quedarán sin sede.',
