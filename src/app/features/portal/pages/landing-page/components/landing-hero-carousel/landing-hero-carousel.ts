...
@Component({
  selector: 'landing-hero-carousel',
  imports: [
    HlmButtonImports,
    HlmCarouselImports,
    NgIcon,
  ...
  template: `
    <section
      class="hero-stage relative overflow-hidden bg-[var(--landing-navy)] text-white"
    ...
    >
      ...
      >
            ...
            <hlm-carousel-item class="pl-0">
              <article class="hero-slide relative overflow-hidden">
                @if (!failedImages().has(slide.id)) {
                  ...
                  <div class="hero-image-fallback absolute inset-0 grid place-items-center" aria-hidden="true">
                    <ng-icon name="lucideImageOff" class="text-6xl text-white/45" />
                  </div>
                }
                    ...
                    @if (slide.linkLabel && validUrl(slide.linkUrl); as url) {
                      <div class="mt-6">
                        @if (isInternalUrl(url)) {
                          <a hlmBtn size="lg" [routerLink]="url" class="w-fit">
                            {{ slide.linkLabel }}
                            <ng-icon name="lucideArrowRight" />
                          </a>
                        } @else {
                          <a hlmBtn size="lg" [href]="url" target="_blank" rel="noopener noreferrer" class="w-fit">
                            {{ slide.linkLabel }}
        ...
        @if (hasMultipleSlides()) {
          ...
          <div class="absolute bottom-4 left-1/2 flex -translate-x-1/2 items-center gap-2 sm:bottom-5" aria-hidden="true">
            @for (slide of slides(); track slide.id; let index = $index) {
              <span
                class="hero-indicator"
                [class.hero-indicator-active]="heroCarousel.currentSlide() === index"
              ></span>
            }
          </div>
          <p class="sr-only" aria-live="polite">
            Contenido {{ heroCarousel.currentSlide() + 1 }} de {{ slides().length }}
          </p>
        }
      </hlm-carousel>
    </section>
  `,
  styleUrl: './landing-hero-carousel.css',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class LandingHeroCarousel {
